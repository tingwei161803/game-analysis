...
一份**程式碼級**的第三方分析網站：把八個 GitHub 專案——craig7351（Book AI）在 36 天內用 AI 打造的五款 3D 瀏覽器遊戲、財團法人開放文化基金會（OCF）維護 22 個月的資安教材，再加上兩款共用同一位設計者的台灣純文字人生模擬（YaKyoLife 棒球人生、CheerLife 啦啦隊人生）——逐行讀過，從**技術深度、遊戲設計、後端與安全、程式品質、測試、完成度、原創性**七大面向各給 1–10 分並算出加權總分。
...
- **分數矩陣熱力圖** — 8 專案 × 7 面向一眼看完，深藍＝越強，滑過任一格看細項。
- **雷達圖輪廓** — 每個專案一張雷達小圖 + 作者平均分疊圖，形狀直接顯示強項與空洞。
- **八頁深度報告** — 每個專案獨立一頁：分數卡、逐面向評語（附檔案:行號證據）、亮點、弱點、AI 輔助開發跡象、統計、總評，以及 kill-switch / prompt-injection 等特別事件。
- **作者對比 + 時間軸** — 四種截然不同的出貨哲學（個人＋AI 極速、組織營運、可量測的平衡工程、繼承格式再換皮），與 craig7351 五作品的開發時間軸。
- **評分方法頁** — 完整規準、權重理由、分析流程、外部佐證與限制聲明，全部公開讓你能反駁每個數字。
- **術語速查** — 可搜尋的技術詞彙表（thin instances、D1、限流、prompt injection…）。
- **中英文各有自己的網址 + 深/淺色** — 英文在根目錄、中文在 `/zh-Hant/`，切換語言＝換一個網址（可加書籤、可分享）；深/淺色仍由 localStorage 記憶。
...
**同一條設計血脈的兩款純文字人生模擬**
7. [LeoGGcat/yakyulife](https://github.com/LeoGGcat/yakyulife) — 棒球生涯模擬（<https://www.yakyolife.com/>）
8. [jarvanthevoyager/CheerLife](https://github.com/jarvanthevoyager/CheerLife) — 啦啦隊生涯模擬（<https://jarvanthevoyager.github.io/CheerLife/>）

> 第 7、8 兩款的 README／片頭都把原創概念署名給同一位設計者（最先生 @mr.themost），本站據此把它們視為同一條設計血脈分析。

所有評語基於實際閱讀原始碼、git 歷史與部署現況（2026-07-20 查核前六個專案的全部 11 個公開網址、2026-08-25 查核兩款人生模擬的部署，皆回應 200）。分數為校準過的判斷，非量測值；規準與證據全部公開於「評分方法」頁。

```
game-analysis/
├── index.html + 11 個 .html   # hub 首頁 + 8 專案頁 + 作者/方法/術語頁（共用 shell）
├── assets/
...
├── data/
│   ├── projects.js            # 八專案的七面向評語、亮點、弱點、統計（雙語）
...
改過 `data/*.js` 之後要重烤靜態內容（每個 .html 都內建一份 JS 渲染結果給爬蟲與無 JS 訪客看，
這支腳本用真的瀏覽器重新產生它，避免兩份內容各走各的）：
...
- 分析為靜態的程式碼級體檢（非動態量測）。前六個專案描述的是 **2026-07-20** 受檢 commit 的狀態，yakyulife 與 CheerLife 為 **2026-08-25**；repo 會前進，分數不代表未來。
